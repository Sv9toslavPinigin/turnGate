...
(function() {
    ...
    if (origFetch) {
        window.fetch = function() {
            ...
            if (typeof urlStr === 'string') { urlStr = rewriteUrl(urlStr); arguments[0] = urlStr; }
    ...
    document.addEventListener('submit', function(e) { if (e.target && e.target.action) e.target.action = rewriteUrl(e.target.action); }, true);
    document.addEventListener('click', function(e) { var t = e.target && e.target.closest ? e.target.closest('a[href]') : null; if (t && t.href) t.href = rewriteUrl(t.href); }, true);

    var origSubmit = HTMLFormElement.prototype.submit;
    HTMLFormElement.prototype.submit = function() { if (this.action) this.action = rewriteUrl(this.action); return origSubmit.apply(this, arguments); };

    rewriteDocument(document);
    if (document.documentElement && window.MutationObserver) {
        new MutationObserver(function(mutations) {
            mutations.forEach(function(m) {
                if (m.type === 'attributes' && m.target) { rewriteElementAttr(m.target, m.attributeName); return; }
                m.addedNodes.forEach(function(n) { if (n.nodeType === 1) rewriteDocument(n); });
            });
...
func solveCaptchaViaProxy(redirectURI string) (string, error) {
	...
	log.Println("[Captcha] Triggering manual captcha fallback...")
	...
	mux.HandleFunc("/generic_proxy", func(w http.ResponseWriter, r *http.Request) {
		...
		targetParsed, parseErr := neturl.Parse(targetAuthURL)
	...
	if startErr := startCaptchaServer(srv, "captcha proxy"); startErr != nil {
		return "", startErr
	}
	...
	timer := time.NewTimer(manualCaptchaTimeout)
	defer timer.Stop()
	...
	case <-timer.C:
		...
		return "", fmt.Errorf("manual captcha timed out after %s", manualCaptchaTimeout)
	}
